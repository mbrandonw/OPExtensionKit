...
+(UIColor*) veryLightGrayColor {
  return MEMOIZE($WAf(0.85f, 1.0f))
}

+(UIColor*) veryDarkGrayColor {
  return MEMOIZE($WAf(0.15f, 1.0f));
}

+(UIColor*) facebookBlue {
  return MEMOIZE($hex(0x3b5998));
}

+(UIColor*) facebookBlueHover {
  return MEMOIZE($hex(0x30487b));
}

+(UIColor*) twitterBlue {
  return MEMOIZE($hex(0x00aced));
}

+(UIColor*) twitterBlueHover {
  return MEMOIZE($hex(0x00a2d0));
}

+(UIColor*) UIKitHighlightBlueTopColor {
  return MEMOIZE($RGBi(5, 140, 245));
}

+(UIColor*) UIKitHighlightBlueBottomColor {
  return MEMOIZE($RGBi(1, 95, 230));
}

+(UIColor*) darkBlueColor {
  return MEMOIZE($RGBi(81, 102, 145));
}
